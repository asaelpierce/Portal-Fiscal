...
export default function ConferenciaFiscal() {
  ...
  const [filtro, setFiltro] = useState('todos') // todos | vinculados | pendentes
  ...
  const filtrados = useMemo(() => {
    ...
    return dados.filter(d => {
      ...
      if (filtro === 'pendentes' && d.vinculado) return false
      if (q) {
        const h = `${d.nf_xml} ${d.nf_compras || ''} ${d.fornecedor} ${d.chave_acesso}`.toLowerCase()
  ...
  const exportar = () => {
    const cols = ['arquivo_xml','nf_xml','nf_compras','chave_acesso','fornecedor','dh_emissao','nunota','dt_neg','vinculado']
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <p style={{ margin: 0, fontSize: 13, color: '#6B7280', maxWidth: 680, lineHeight: 1.6 }}>
        ...
        (compras). A data de início é sempre o primeiro dia do mês atual por padrão — ajuste se quiser olhar
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
            {[
              ...
              { label: 'Sem vínculo', valor: int(meta.nao_vinculados), sub: 'precisa lançar/verificar', cor: meta.nao_vinculados > 0 ? '#B54708' : '#12805C' },
            ].map((k, i) => (
          ...
          >
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 14, alignItems: 'flex-end' }}>
              <div style={{ display: 'flex', gap: 6 }}>
                {[
                  ...
                  { id: 'pendentes', label: `Pendentes (${dados.filter(d => !d.vinculado).length})`, cor: '#B54708' },
            ...
            <div style={{ maxHeight: 560, overflowY: 'auto', overflowX: 'auto', margin: '0 -18px -16px', borderTop: '1px solid #F3F4F6' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
                <thead>
                  <tr>
                    {['Arq. XML', 'NF (XML)', 'NF (Compras)', 'Fornecedor', 'Emissão', 'Status', 'Chave de acesso'].map(h => (
                      <th key={h} style={{
                ...
                <tbody>
                  {filtrados.map((d, i) => (
                    <tr key={i} style={{ background: d.vinculado ? '#F0FDF4' : '#FFFBEB' }}>
                      ...
                      <td style={{ padding: '8px 12px', color: '#6B7280', maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={d.fornecedor}>
                        {d.fornecedor}
                      </td>
                  ...
                  {!filtrados.length && (
                    <tr><td colSpan={7} style={{ textAlign: 'center', padding: '28px', color: '#9CA3AF' }}>
